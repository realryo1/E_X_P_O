...
* ゲームメニューの「フォトモード」を有効にすると、機体操作と三人称追従を止め、WASD / Space / Shift と右クリック視点、右スティックでカメラを動かす。メニュー中はフリーカメラ入力を止める
* フォトモードは `Esc` / パッドSTARTで終了し、カメラ・ライティング・エフェクトの設定を入場前へ戻す。レース中は計測と通過判定を一時停止する
