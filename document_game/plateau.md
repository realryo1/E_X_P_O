...
- LOD3近景はカメラ周辺と移動方向の先読み範囲を距離ストリーミングする。開始はロード半径、GPU化は破棄半径まで進め、ヒステリシス帯のREADYが新規インポートを止めない。距離判定には既知のモデルXZ半径を足す。万博GLBはワーカーでGLB 2.0のAccessor/BufferViewを直接展開し、埋め込みテクスチャのCPUデコードもワーカーで行う。GPU化と破棄は1フレーム6msの予算で進め、頂点・インデックス・テクスチャの転送はチャンク化する。GPU化中は`cube.fbx`のプレースホルダーを表示する
...
距離ストリーミング判定には使わない。
現在は距離と移動方向の先読みでLOD3のロード対象を絞り、不要になったGPUバッファと
テクスチャを破棄する。開始はロード半径 24、GPU化は破棄半径 36、距離には既知のモデルXZ半径を足す。
CPUデコードはワーカーへ移し、GPUアップロードと破棄はゲーム更新中の6ms予算に分割する。
