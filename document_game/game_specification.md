...
- `Game_Draw`: 初期ロード中は場の3DとSSAOを描かず、フェードとHUDだけにする。完了後は `Direct3D_BeginScene` で内部解像度のシーン色RTへ切り替え、`PlayerCamera_Draw` のあと、深度ありで `Sunlight_Apply`、注視点周辺の局所 ShadowMap（床・パンチ済みLOD2・遠景LOD2・リング・タクシーを投影。遠景はセル無しのメッシュ単位）、null2 の環境キューブ1面、場とプレイヤーの描画を行う。続いて `Direct3D_ApplySsao` で内部RTをバックバッファへ拡大する。SSAOがオンのときだけ1/4解像度AOを合成する。マテリアルを白へ戻してから `Ui_Draw`、HUD、ゲーム内メニュー、Debugビルド用の`Player_DrawDebug` / `Sunlight_DrawDebug` / `PlayerCamera_DrawDebug`を描画する。SSAOは3D色だけを対象とし、UIには適用しない。
