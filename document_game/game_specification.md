...
| Game | W / 左スティック上で前進、Spaceで上昇、Shiftで下降、マウス左右で機体旋回、右スティックで視点。`Esc` / パッドSTARTでメニューを開閉し、矢印キー / 十字キーとEnter / パッドAでメニューを操作する。メニュー中は機体操作を停止する。上下移動は前進中なら斜め、停止中なら垂直になる。A/Dの横移動とSの後退は行わない。コース作成中は`P`で輪を配置し、`U`で最後の輪を取り消す |
